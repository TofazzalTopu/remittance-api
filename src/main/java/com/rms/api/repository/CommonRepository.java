...
public class CommonRepository {
    ...
    public List<RemittanceData> getRemittanceDataForNotification() {
        try {
            // this query has to be updated for RIA
            TypedQuery<RemittanceData> query = em.createQuery("SELECT r FROM RemittanceData r  where r.middlewarePush = 0 and r.sourceType=:source_type and r.processStatus IN ('COMPLETED','REJECTED')", RemittanceData.class);
