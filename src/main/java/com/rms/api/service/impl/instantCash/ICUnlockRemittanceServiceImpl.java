...
import com.rms.api.service.impl.common.RemittanceDataProcessServiceImpl;
...
public class ICUnlockRemittanceServiceImpl implements ICUnlockRemittanceService {
    ...
    public void unlockICOutstandingRemittance(ICExchangePropertyDTO icDTO) {
        ...
        try {
            List<RemittanceData> remittanceDataList = remittanceDataService.findAllByExchangeCodeAndSourceTypeAndProcessStatus(icDTO.getExchangeCode(), Constants.API_SOURCE_TYPE, RemittanceData.UNLOCK_REQUESTED);
            remittanceDataList.forEach(remittanceData -> {
                try {
                    ...
                    if ((responseEntity.getStatusCode().equals(HttpStatus.OK)) && Objects.nonNull(responseEntity.getBody())) {
                        remittanceDataService.update(RemittanceData.UNLOCK, remittanceData.getReferenceNo());
